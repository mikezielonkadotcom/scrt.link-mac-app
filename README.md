...
- **Recent-secrets sidebar** — every link you create is stored locally with its label, type, and expiration, with one-click re-copy.
...
- **Local history** — last 100 secrets, no plaintext stored (only the link + metadata). Wipeable from Preferences or the sidebar.
...
- Shows recent links with label · type · relative time · expiration
...
- **Clear History** wipes local records (doesn't affect the secrets on scrt.link)
